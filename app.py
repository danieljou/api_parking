...
@app.route("/parking/all", methods=["GET","POST"])
def parkingss():
    conn = db_connection()
    cursor = conn.cursor()

    if request.method == "GET":
        cursor.execute("SELECT * FROM PARKING_SMART WHERE id IN (SELECT MAX(id) FROM PARKING_SMART GROUP BY parking_id)")
...
def rechercher_parking(id):
    ...
    if request.method== "GET":
        cursor.execute("SELECT * FROM PARKING_SMART WHERE slot_id=?",(id,))
        parkings=[
